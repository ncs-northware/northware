...
- **`drizzle-generate`** navigiert in den Ordner `./packages/database` und führt dort `drizzle-kit generate` aus.
- **`drizzle-migrate`** navigiert in den Ordner `./packages/database` und führt dort `drizzle-kit migrate` aus.
