...
<Meta title="Components/ThemeProvider" />

# ThemeProvider

Der `ThemeProvider` umgibt das Root-Layout der App über den [GeneralProvider](?path=/docs/components-generalprovider--docs).
...
<ViewSource path="components/theme-provider.tsx" />
...
Um zwischen den Themes wechseln zu können, kann der [ThemeSwitch](?path=/docs/components-themeswitch--docs) verwendet werden.
