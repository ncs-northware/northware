...
Die DataTable basiert auf [TanStack Table](https://tanstack.com/table/latest/docs/overview), erweitert durch shadcn/ui
...
Da jede Tabelle unterschiedliche Anforderungen hat und auf unterschiedliche Weise funktioniert, 
wird die `DataTable` nicht in einer einzelnen Komponente zusammengefasst.
Ich folge hier dem Ansatz von `shadcn/ui`. Demnach sollte für jede Seite (oder für einen Teil der App), die `DataTables` verwenden eine eigene Komponente erstellt werden.
...
- Bei `shadcn` ist außerdem eine `data-table-features.ts` für jede DataTable vorgesehen. Das Objekt `features` mit dem dazugehörigen Typ `DataTableFeatures` sagt TansStack Table, welche Features verwendet werden sollen. In meinem Setup sind `features` und `DataTableFeatures` im Component-Code von `@northware/ui` enthalten. Somit können alle Tabellen die gleichen Features verwenden, sofern die UI daruaf zugreift.

### Column Definitions

In der `columns.tsx` wird definiert welchem TypeScript Type die Daten folgen (`Payment`).
Außerdem werden die Spalten definiert. 

Der einfachste Weg ist ein `accessor`, der einem Objekt im Type folgt und ein `header` mit einem String nach dem die Spalte benannt ist:
...
Hier werden über das Objekt `features` die Funktionen integriert, die innerhalb der Komponente abgerufen werden.
...
und nicht in der `data-table.tsx` definiert. `<table.FlexRender>` ist daher jeweils das einzige und direkte Child der `Array.map()` Abfragen und braucht ein `key` Element.
...
### features bzw. DataTableFeatures

Folgende Features sind über das Objekt `features` bzw. den abgeleiteten Typ `DataTabeleFeatures` eingebunden:
...
Die Pagination ist über `createPaginationRowModel` in `features` bereits aktiviert

Es gibt auch noch weitere Anpassungsmöglichkeiten. [Pagination Docs](https://tanstack.com/table/latest/docs/framework/react/guide/pagination)
...
Die Komponente `<DataTableFilter />` fügt der `DataTable` ein Filter-Input hinzu. Über dieses Eingabefeld kann die Tabelle über alle Spalten hinweg durchsucht werden.

<Canvas of={DataTableStories.Filter} />

Für die Verwendung der Komponente muss `useTable` zusätzlich zu den `features` einige Definitionen enthalten. 
...
Die Komponente `DataTableFilter` erwartet das Poperty `table` und greift darüber auch den State von `globalFilter` ab:
...
Mehr zu [Global Filtering in der TanStack Table](https://tanstack.com/table/latest/docs/framework/react/guide/global-filtering)

Es gibt auch die Möglichkeit, die Filter-Funktion nur für einzelne Spalten einzurichten. 
Mehr dazu bei [shadcn](https://ui.shadcn.com/docs/components/base/data-table#filtering) und bei [TanStack Table](https://tanstack.com/table/latest/docs/framework/react/guide/column-filtering).
...
Für die Verwendung der `DataTableViewOptions` muss `useTable` den State der sichtbaren Spalten verwalten.
...
Um das sortieren über die Spaltenüberschriften zu ermöglichen, muss in `useTable` der Sorting State kontrolliert werden.
...
Das `useTable` Objekt muss den Select State kontrollieren.
...
Die Spalte, die einen Selector enthält, enthält keine Daten. Daher muss hier `columnHelper.display()` statt `columnHelper.accessor()` verwendet werden.
...
Eine der Spalten müssen auf allen Geräten angezeigt werden. Hier enthält die `cell` eine `TableDescriptionList` die alle Daten anzeigt, 
