...
Um ein weiteres Icon zu `@northware/ui/icons` hinzuzufügen, muss die SVG-Datei des Icons im Ordner `/packages/ui/src/icons/assets` abgelegt werden.
...
```bash
svgr --config-file svgr.config.js --ignore-existing -- ./src/icons/assets
```
