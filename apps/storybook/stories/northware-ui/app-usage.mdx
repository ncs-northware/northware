...
- Über das `<body>` Tag wird die Schriftart definiert. Die Variable fonts übergibt die Schriftarten in den Variablen `SourceSans` und `SourceCode` an die App und definiert, das für die App standardmäßig die Klasse `.font-sans` verwendet werden soll. [Mehr dazu](?path=/docs/northware-ui-utilities-und-funktionen--docs#fonts)
